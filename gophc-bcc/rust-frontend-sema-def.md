...
### impl<'a> SemanticAnalyzer<'a> 方法签名
...
### impl<'a> InitializerAnalyzer<'a> 方法签名
...
### impl<'a> ConstantEvaluator<'a> 方法签名
...
### impl<'a> BuiltinEvaluator<'a> 方法签名
...
### impl<'a> AttributeHandler<'a> 方法签名
...
1. `mod.rs` - SemanticAnalyzer 主结构
2. `type_checker.rs` - TypeChecker 类型检查器
3. `symbol_table.rs` - SymbolTable 符号表
4. `scope.rs` - ScopeStack 作用域管理
5. `initializer.rs` - InitializerAnalyzer 初始化分析器
6. `constant_eval.rs` - ConstantEvaluator 常量求值器
7. `builtin_eval.rs` - BuiltinEvaluator 内建函数求值器
8. `attribute_handler.rs` - AttributeHandler 属性处理器